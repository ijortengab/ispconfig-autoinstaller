...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --digitalocean-token=*|-D=*) digitalocean_token="${1#*=}"; shift ;;
        --digitalocean-token|-D) if [[ ! $2 == "" && ! $2 =~ ^-[^-] ]]; then digitalocean_token="$2"; shift; fi; shift ;;
